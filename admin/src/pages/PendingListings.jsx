...
const PendingListings = () => {
  ...
  const fetchProperties = async () => {
    try {
      setLoading(true);
      const response = await axios.get(`${backendurl}/api/products/pending`);

      if (response.data?.success) {
        // Use response.data.data instead of response.data.property
        const propertiesArray = response.data.data || []; // Fallback to empty array
  ...
  const handleApproveProperty = async (propertyId, propertyTitle) => {
    ...
    try {
      ...
      setProperties((prev) =>
        prev.map((p) =>
          p._id === propertyId ? { ...p, approvalStatus: "approved" } : p
        )
      );
    ...
    try {
      if (typeof amenities[0] === "string") {
        ...
        if (firstAmenity.startsWith("[") && firstAmenity.endsWith("]")) {
          ...
          // It's just a normal array of strings
          return amenities;
        }
      } else {
        return amenities;
  ...
  const handleEditProperty = (id) => {
    navigate(`/update/${id}`); // This will navigate to the "NextPage" component
  };
